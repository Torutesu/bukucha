...
## 2026-08-13 追補6: ホーム/詳細をZeta実ページ構造に刷新

- 参照: Zeta実プロフィールページ(fetch調査)+ホームのスクリーンショット(ユーザー提供。コンテストタブは除外指定)
- ホーム(SCR-002 v2): トレンド/ベスト/新作タブ+カテゴリチップ(その場絞り込み)+2カラム大判グリッド。カードは大表紙+ランクバッジ+💬会話数バッジ(万表記)+タイトル+一言+#タグ+@作者
- 詳細(SCR-005 v2): 統計を万表記(6.2万形式)に、タイトル直下にキャッチコピー、のぞき見をシーンプレビュー化(導入地の文→キャラ名チップ→最初のセリフ)、関連作品レール「こんな物語も」(先頭タグ一致)、公開日表記
- cardSelectにauthor追加。E2E-003/004/011/013をタブUI仕様に更新。全32件green

## 2026-08-13 追補7: アカウント登録・マイページ群(Zetaスクリーンショット準拠)

- 参照: ユーザー提供のZeta実画面(アカウント登録/規約同意シート/マイページ/プロフィール編集/詳細ハブ/作成タブ/トーク一覧)
- 登録(SCR-017 v2): 新規ユーザーのみ「キャラクターに呼んで欲しい名前」入力→規約同意ボトムシート
  (すべて同意する + 必須2/任意1)。同意時に nickname 保存 + デフォルトのトークプロフィール(Persona)作成
- User に handle(@ID, unique)/bio/agreedAt を追加。/api/me PATCH で bio(100字)・handle(検証、重複は409)・agreeTerms
- マイページ(SCR-014 v2): プロフィールカード(アバター/@ID/フォロー統計)、プロフィール共有(Web Share→クリップボード)、
  Bukuchaパス/マイピース(準備中)、いいねレール、規約フッター、☰→/me/details
- /me/edit: ソーシャルプロフィール(アバター/クリエイター名/ID/自己紹介0-100)と トーク用(ペルソナ)のタブ
- /me/details: 設定ハブ(リワード/カスタマーセンター/自分の活動/通知/アカウント設定/ブロック管理/ログアウト)。
  未実装項目は「準備中」トーストで明示
- 下部タブ: 本棚→トーク、作る→作成(/studio)。作成タブは 公開/非公開/未登録 + 修正日ソート + FAB、
  トーク一覧は丸アバター+最終メッセージ+相対時刻の行レイアウト

## 2026-08-13 追補8: プロットエディタ(Zeta 6タブ)とスタイル/設定集

- /create を全画面エディタに再構成: プロンプト / 設定集 / スタイル / イントロ / 紹介 / 設定。
  ヘッダー = ✕ / 一時保存 / 完成。必須未入力タブに❗。初回のみ注意事項ボトムシート
- Situation に style(JSON) / lore(JSON) / commentsEnabled / creatorComment を追加
- スタイル → プロンプト規則に変換(src/lib/plot-style.ts): 視点・時制・応答の長さ・表現方式・難易度・
  展開速度・雰囲気・文章スタイル・インフォボックス。NOVEL_RULESから視点/字数の固定記述を撤去し、スタイル駆動に
- 設定集: キーワード+内容を最大5件。直近6メッセージ+入力+導入文にキーワードが出たときだけ system に注入
  (全件常時注入だとトークンを食い、無関係な設定が混ざるため)
- エディタの保存は直列化。並行PATCHだと到着順の入れ替わりで後の変更が古い値に上書きされる事故があった(E2E-013で検出)
- mock provider の debug に activeLore を追加、test-turn の done イベントで debug を返して注入をE2Eで検証(E2E-043)
